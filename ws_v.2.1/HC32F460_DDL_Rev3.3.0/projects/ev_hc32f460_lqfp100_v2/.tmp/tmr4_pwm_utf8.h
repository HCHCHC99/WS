...
/* PWM channel pair (U / V / W) 鈥 named to avoid collision with DDL TMR4_PWM_CH_* macros */
typedef enum {
    TMR4_CHANNEL_U = 0,
    TMR4_CHANNEL_V = 1,
    TMR4_CHANNEL_W = 2,
...
/* Set PWM frequency (Hz) 鈥 updates counter period. All duties must be re-applied after. */
 ...
 * Writes only that pin's OCCR 鈥 the other pin is unchanged. */
 ...
 * Use for OFF phase: set H=HIGH, L=LOW 鈫 disable both 鈫 H,L 鈫 interlock. */
