...
void TMR4_PWM_Config(const tmr4_pwm_config_t *pConfig)
{
    ...
    MAIN_D("[TMR4] All 3 channels configured: mode=%s period=%u polar=%s",
           (pConfig->output_type == TMR4_OUTPUT_COMPLEMENTARY) ? "COMP" : "SYNC",
           s_u16Period,
...
void TMR4_PWM_PinCmd(tmr4_pwm_channel_t channel, bool high_side, bool enable)
{
    ...
    if (s_output_type == TMR4_OUTPUT_COMPLEMENTARY && high_side) {
        return;
    }
...
/*=============================================================================
 * TMR4_PWM_SetChannelComplementary - Switch a channel to complementary PWM
 *=============================================================================*/
void TMR4_PWM_SetChannelComplementary(tmr4_pwm_channel_t channel, uint16_t dead_time_ns)
{
    ...
    pair = &s_pairs[channel];
    u32TimerClock = CLK_GetBusClockFreq(CLK_BUS_PCLK1);
    u16DeadTicks = DeadTimeNsToTicks(dead_time_ns, u32TimerClock);

    /* Re-init low-side OC for complementary mode (high side auto-generated) */
    TMR4_OC_StructInit(&stcOcInit);
    TMR4_OC_Init(CM_TMR4_3, pair->oc_ch_l, &stcOcInit);
    OCMode_CompLow(CM_TMR4_3, pair->oc_ch_l);

    /* Re-init PWM for DEAD_TMR mode */
    ...
    /* High side OC disabled (auto-generated by dead-timer in complementary mode) */
    TMR4_OC_Cmd(CM_TMR4_3, pair->oc_ch_h, DISABLE);
