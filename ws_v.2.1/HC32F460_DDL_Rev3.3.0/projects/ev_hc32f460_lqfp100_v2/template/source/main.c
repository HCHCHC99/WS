...
//   }
  ...
  #define OL_START_INTERVAL_US    20000UL  /* Æð²½ ~167 RPM, µÍËÙ´óÅ¤¾Ø */
  ...
  /* Hall Ó³Éä±íË÷Òý: Keil Watch ¿É¸Ä, ¸÷Ä£Ê½¶ÀÁ¢ */
  volatile int hall_table_cw  = 14;  /* mode 3 ±Õ»·Õý×ªÓÃ */
  ...
  int main(void)
  {
    ...
    //   Motor_Pwm_Init();
      ...
      while (1) {
          ...
          /* Ä£Ê½3/4: ¿ª»·Ç¿ÍÏ ¡ú ·ÉÐÐÆô¶¯ÇÐ±Õ»· */
          if (comm_mode == 3 || comm_mode == 4) {
              Timer6_Timebase_UpdateTimestamp();
              uint64_t now = Timer6_Timebase_GetTimestamp();

              if (comm_sub_phase == 0) {
                  /* === ½×¶Î0: ¿ª»·Ç¿ÍÏ + Ð±ÆÂ (OL_START ¡ú OL_TARGET) === */
                  ...
                  /* Ð±ÆÂÍê³É ¡ú ·ÉÐÐÆô¶¯ÇÐ±Õ»· (¶Á Hall µ±Ç°Î»ÖÃ, Ìø¹ý¶ÔÆë) */
                  if (ramp_elapsed >= ramp_total) {
                      if (comm_mode == 3) {
