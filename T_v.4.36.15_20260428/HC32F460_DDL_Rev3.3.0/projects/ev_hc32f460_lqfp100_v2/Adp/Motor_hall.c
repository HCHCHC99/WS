...
typedef struct {
    ...
    volatile uint32_t hall_pulse_counts[
...
#if MOTOR_HALL_TRIPLE_ENABLE
    if (inst->config.eirq_ch_c != 0) count++;
#endif
    return count;
}
...
#if MOTOR_HALL_TRIPLE_ENABLE
static void hall_c_irq_callback(void)
{
    motor_hall_instance_t* inst = g_current_hall_c_instance;
    if (!inst || !inst->valid) return;

    EXTINT_ClearExtIntStatus(inst->config.eirq_ch_c);

    inst->hall_pulse_counts[2]++;
...
static void check_hall_status(motor_hall_instance_t* inst)
{
    ...
    if (current_time - last_check_time[inst->id] >= HALL_CHECK_INTERVAL_MS) {
        ...
        /* determine active hall count from config */
        uint8_t cfg_hall_count = calculate_active_hall_count(inst);

#if MOTOR_HALL_TRIPLE_ENABLE
        /* count active halls by pulse activity */
        uint8_t active_by_pulse = 0;
        uint32_t counts[3];
        counts[0] = inst->hall_pulse_counts[0];
        counts[1] = inst->hall_pulse_counts[1];
        counts[2] = inst->hall_pulse_counts[2];

        uint8_t pulse_active[3] = {0};
        for (int h = 0; h < 3; h++) {
            if (cfg_hall_count > (uint8_t)h && counts[h] > 0) {
                pulse_active[h] = 1;
                active_by_pulse++;
            }
        }

        if (active_by_pulse == 0) {
            inst->hall_status = HALL_STATUS_NONE;
            inst->active_hall_count = 0;
        } else if (active_by_pulse == 1) {
            if (pulse_active[0])      { inst->hall_status = HALL_STATUS_A_ONLY; }
            else if (pulse_active[1]) { inst->hall_status = HALL_STATUS_B_ONLY; }
            else                      { inst->hall_status = HALL_STATUS_ERROR; }
            inst->active_hall_count = 1;
        } else {
            /* check balance across all active halls */
            uint8_t max_ok = 0;
            uint8_t failed = 0;
            for (int h = 0; h < 3; h++) {
                if (!pulse_active[h]) continue;
                uint32_t total = 0;
                for (int k = 0; k < 3; k++) {
                    if (pulse_active[k]) total += counts[k];
                }
                if (total == 0) break;
                /* compare this hall's count to the average of others */
                uint32_t avg_others = (total - counts[h]) / (active_by_pulse - 1);
                uint32_t diff = (counts[h] > avg_others) ? (counts[h] - avg_others) : (avg_others - counts[h]);
                if (avg_others > 0 && (diff * 100 / avg_others) <= HALL_WARNING_THRESHOLD) {
                    max_ok++;
                } else if (avg_others == 0) {
                    max_ok++;
                } else {
                    failed++;
                }
            ...
            inst->active_hall_count = active_by_pulse;
        }
#else
...
motor_hall_handle_t motor_hall_create(const motor_hall_config_t* config)
{
...
#if MOTOR_HALL_TRIPLE_ENABLE
    inst->config.hall_c_port = config->hall_c_port;
    inst->config.hall_c_pin = config->hall_c_pin;
    inst->config.eirq_ch_c = config->eirq_ch_c;
    inst->config.irqn_c = config->irqn_c;
    inst->config.irq_src_c = config->irq_src_c;
#endif
    ...
    if (config->eirq_ch_c != 0) {
        /* Register Hall C IRQ */
        {
            ...
            stcExtiConfig.u32FilterClock = EXTINT_FCLK_DIV1;
            stcExtiConfig.u32Edge = EXTINT_TRIG_BOTH;
            EXTINT_Init(config->eirq_ch_c, &stcExtiConfig);
            ...
            stcPortInit.u16PullUp = PIN_PU_ON;
            LL_PERIPH_WE(LL_PERIPH_GPIO);
            GPIO_Init(config->hall_c_port, config->hall_c_pin, &stcPortInit);
            GPIO_ExtIntCmd(config->hall_c_port, config->hall_c_pin, ENABLE);
            LL_PERIPH_WP(LL_PERIPH_GPIO);

            stcIrqRegiConf.enIntSrc = (en_int_src_t)config->irq_src_c;
            stcIrqRegiConf.enIRQn = (IRQn_Type)config->irqn_c;
            stcIrqRegiConf.pfnCallback = (func_ptr_t)hall_c_irq_callback;
            INTC_IrqSignIn(&stcIrqRegiConf);

            EXTINT_ClearExtIntStatus(config->eirq_ch_c);
            NVIC_ClearPendingIRQ((IRQn_Type)config->irqn_c);
            NVIC_SetPriority((IRQn_Type)config->irqn_c, config->irq_priority);
            NVIC_EnableIRQ((IRQn_Type)config->irqn_c);

            g_current_hall_c_instance = inst;
        }
    }
#endif
...
uint32_t motor_hall_get_hall_c_count(motor_hall_handle_t handle)
{
    if (!handle) return 0;
    motor_hall_instance_t* inst = (motor_hall_instance_t*)handle;
    return inst->hall_pulse_counts[2];
}
#endif
...
uint32_t motor_hall_get_total_pulse_count(motor_hall_handle_t handle)
{
    ...
    motor_hall_instance_t* inst = (motor_hall_instance_t*)handle;
#if MOTOR_HALL_TRIPLE_ENABLE
    return inst->hall_pulse_counts[0] + inst->hall_pulse_counts[1] + inst->hall_pulse_counts[2];
#else
